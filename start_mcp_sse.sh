...
# Simple script to start the MCP server with SSE transport
# This focuses on properly configuring the server without any testing
...
LOG_LEVEL="INFO"
LOG_FILE="./mcp_sse_$(date +"%Y%m%d_%H%M%S").log"
...
echo -e "${BLUE}===== Starting MCP Server with SSE Transport =====${NC}"
...
python3 server.py --transport sse --port $PORT --host "0.0.0.0" --log-level $LOG_LEVEL | tee "$LOG_FILE"
